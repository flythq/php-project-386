...
@section('content')
    <section class="max-w-lg">
        ...
        <form method="POST" action="{{ route('availabilities.store') }}" class="mt-8 space-y-4">
            @csrf
            <div>
                ...
                <select name="weekday" id="weekday" class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-gray-900 dark:text-gray-100">
                    @for ($d = 1; $d <= 7; $d++)
                        <option value="{{ $d }}" {{ old('weekday') == (string) $d ? 'selected' : '' }}>{{ $d }}</option>
                    @endfor
                </select>
                @error('weekday')
            ...
            <div>
                ...
                <input type="text" name="start_time" id="start_time" value="{{ old('start_time') }}" placeholder="10:00" class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-gray-900 dark:text-gray-100">
                @error('start_time')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="end_time" id="end_time" value="{{ old('end_time') }}" placeholder="18:00" class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-gray-900 dark:text-gray-100">
                @error('end_time')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
